...
pivot['log_assets_geo'] = np.log(pivot['total assets'])
...
model3_numerisk = smf.ols('andel_smv ~ bank_size_numeric + time_trend + return_on_assets + npl_ratio + net_interest_income_ratio + leverage_ratio',data=pivot).fit(cov_type='HC1')
...
first_stage = smf.ols('log_assets_geo ~ C(geografisk_eksponering) + time_trend + return_on_assets + npl_ratio + net_interest_income_ratio + leverage_ratio', data=pivot).fit(cov_type='HC1')
pivot['log_assets_hat'] = first_stage.fittedvalues
second_stage = smf.ols('andel_smv ~ log_assets_hat + time_trend + return_on_assets + npl_ratio + net_interest_income_ratio + leverage_ratio',data=pivot).fit(cov_type='HC1')
...
Model5_7 = summary_col(
    results=[model3_numerisk, model5, model6, second_stage],
    model_names=["Model 3 numerisk", "Model 5", "Model 6", 'Model 7 IV'],
    stars=True,
    float_format="%.3f",
    info_dict={
        'R-squared': lambda x: f"{x.rsquared:.3f}",
